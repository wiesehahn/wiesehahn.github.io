...
plot_leafoff <- function(dat){
  dat|>  
  mutate(Year = if_else(month %in% c("Nov", "Dez", "Jan", "Feb", "Mrz"), 
                                  as.character(year), 
                                  NA_character_)) |> 
  arrange(date) |> 
  ggplot() + 
...
```{r}
#| echo: false
#| fig-cap: Available coverage of image based height model in Lower Saxony during leaf-off season (Nov - Mar)
...
plot_leafon <- function(dat){
  dat|>
  mutate(Year = if_else(month %in% c("Apr", "Mai", "Jun", "Jul", "Aug", "Sep", "Okt"), 
                                  as.character(year), 
                                  NA_character_)) |> 
  arrange(date) |> 
  ggplot() + 
...
```{r}
#| echo: false
#| fig-cap: Available coverage of image based height model in Lower Saxony during leaf-on season (Apr - Oct)
